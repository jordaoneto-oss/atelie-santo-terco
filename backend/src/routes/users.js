...
router.post('/', adminOnly, asyncHandler(async (req, res) => {
  ...
  const result = await db.prepare('INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, ?)').run(name, email, hash, role || 'viewer');
  const user = await db.prepare('SELECT id, name, email, role, created_at FROM users WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', adminOnly, asyncHandler(async (req, res) => {
  ...
  if (password) {
    const hash = bcrypt.hashSync(password, 10);
    await db.prepare('UPDATE users SET name=?, email=?, password_hash=?, role=? WHERE id=?').run(name ?? user.name, email ?? user.email, hash, role ?? user.role, req.params.id);
  } else {
    await db.prepare('UPDATE users SET name=?, email=?, role=? WHERE id=?').run(name ?? user.name, email ?? user.email, role ?? user.role, req.params.id);
  }
